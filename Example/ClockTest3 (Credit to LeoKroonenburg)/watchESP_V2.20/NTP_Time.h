...
#define TIMEZONE idWIB // See below for other "Zone references", UK, usMT etc
...
const char* ntpServerName = "id.pool.ntp.org";                                          // I am in indo, so the Indonesia server pool //Edited by CalliArca 09/02/2024
#else
// Try to use pool url instead so the server IP address is looked up from those available
// (use a pool server in your own country to improve response time and reliability)
// const char* ntpServerName = "time.nist.gov";
const char* ntpServerName = "id.pool.ntp.org";              //Edited by CalliArca 09/02/2024
//const char* ntpServerName = "time.google.com";
#endif
...
/* //edited by CalliArca 09-02-2024
...
//edited by CalliArca 09-02-2024
// Zone reference "idWIB" Western Indonesia Time (Jakarta)
TimeChangeRule WIB = {"WIB", Last, Sun, Mar, 2, 420};  // Western Indonesia Time
TimeChangeRule WITA = {"WITA", Last, Sun, Mar, 2, 480}; // Central Indonesia Time
TimeChangeRule WIT = {"WIT", Last, Sun, Mar, 2, 540};   // Eastern Indonesia Time
Timezone idWIB(WIB, WIB); // Western Indonesia Time zone reference
Timezone idWITA(WITA, WITA); // Central Indonesia Time zone reference
Timezone idWIT(WIT, WIT); // Eastern Indonesia Time zone reference
...
void decodeNTP(void) {
    ...
    while (millis() < waitTime && !timeValid){
      ...
      local = idWIB.toLocal(utc, &tzCode);                                // convert utc into local timezone compensated
